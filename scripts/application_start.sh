...
export DB_HOST=$(aws ssm get-parameter --name "AIRFLOW_POSTGRES_HOSTNAME" --query "Parameter.Value" --output text)
export DB_PORT=5432
...
export AIRFLOW__WEBSERVER__SECRET_KEY=$(aws ssm get-parameter --name "AIRFLOW__WEBSERVER__SECRET_KEY" --with-decryption --query "Parameter.Value" --output text)
